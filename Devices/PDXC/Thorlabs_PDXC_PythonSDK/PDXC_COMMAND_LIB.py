...
class pdxc:
    ...
    def __init__(self):
        lib_path = "./Devices/PDXC/Thorlabs_PDXC_PythonSDK/PDXC_COMMAND_LIB_win64.dll"
        if not pdxc.isLoad:
    ...
    def GetTargetTriggerPosition(self, secondary, position):
        ...
        Args:
            secondary: index in daisy chain (0:Single Mode or Main, 1 -11 : Secondary1 - Secondary11)
            position: target position: PDX1[-10,10]; PDX2[-2.5,2.5]mm; PDXR:[-999.9,999.9]
        Returns:
            0: Success; negative number: failed.
        """
        ret = -1
    ...
    def GetAllParametersInExternalTrigger(self, secondary, alltriggerState):
        ...
        rising / falling edge and value defined in PDX1(mm)/PDX2(mm)/PDXR(°) unit, and Two - Position - Switching
        mode with rising / falling
        for each position, assigned by pos1 and pos2.
        Args:
            secondary: index in daisy chain (0:Single Mode or Main, 1 -11 : Secondary1 - Secondary11)
    ...
    def SetAnalogInputGain(self, secondary, aiGain):
        ...
        standing for PDX1[+ -10mm]/PDX2[+ -2.5]PDXR[+ - 10°] position range.If change value of Gain&Offset,
    ...
    def SetPositionLimit(self, secondary, minvalue, maxvalue):
        ...
        Args:
            secondary: index in daisy chain (0:Single Mode or Main, 1 -11 : Secondary1 - Secondary11)
            minvalue: Minimum value of position, PDX1 range[-10,max] mm; PDX2 range[-2.5,max] mm;
            PDXR range[-999.9,max] °
            maxvalue: Maximum value of position, PDX1 range[min,10] mm; PDX2 range[min,2.5] mm;
